...
from app.services.talos_audit.callback import (
    build_talos_completion_payload,
    report_talos_progress,
    send_talos_completion,
)
...
@pytest.mark.asyncio
async def test_report_talos_progress_posts_expected_contract(monkeypatch):
    captured: dict[str, object] = {}
    ...
    monkeypatch.setattr(callback_module.settings, "TALOS_PROGRESS_CALLBACK_URL", "http://talos.test/api/v1/callback/scanner")
    monkeypatch.setattr(callback_module.settings, "TALOS_CALLBACK_TOKEN", "callback-secret")
    monkeypatch.setattr(callback_module.httpx, "AsyncClient", FakeAsyncClient)

    assert await report_talos_progress(
        taskid="task-1",
        status="running",
        progress=50,
        stage="scan",
